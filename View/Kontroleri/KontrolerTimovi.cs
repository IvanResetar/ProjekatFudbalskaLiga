...
namespace View.Kontroleri
{
    public class KontrolerTimovi
    {
        ...
        private BindingList<Igrac> napadaci;
        ...
        private DialogUtakmiceTima dialogUtakmiceTima;

        private DialogNapadaciTima dialogNapadaciTima;

        private DialogDodajDrzavu dialogDodajDrzavu;
        ...
        internal void OtvoriDialogUtakmiceTima(Tim timDto)
        {
            Tim tim = (Tim)Komunikacija.Komunikacija.Instance.VratiObjekat(Operacije.VratiObjekatTim, (object)timDto)[0];
            if (tim.TimID == 0)
            {
                MessageBox.Show("Sistem ne može da učita tim");
                return;
            }
            this.dialogUtakmiceTima = new DialogUtakmiceTima(this, tim);
            this.dialogUtakmiceTima.ShowDialog();
        }
        ...
        internal void InicijalizujDialogUtakmiceTima(DialogUtakmiceTima dialogUtakmiceTima, Tim tim)
        {
            utakmice = VratiListuUtakmicaTima(tim);
            dialogUtakmiceTima.DataGridUtakmice.DataSource = utakmice;
        }

        private BindingList<Utakmica> VratiListuUtakmicaTima(Tim tim)
        {
            ...
            {
                if((u.DomacinID.TimID == tim.TimID || u.GostID.TimID == tim.TimID)
                    && u.GostGolovi >=0 && u.DomacinGolovi >= 0)
                {
                    utakmice.Add(u);
                }
            }
        ...
        internal void OtvoriDialogNapadaciTima(Tim tim)
        {
            this.dialogNapadaciTima = new DialogNapadaciTima(this, tim);
            this.dialogNapadaciTima.ShowDialog();
        }

        internal void InicijalizujDialogNapadaciTima(DialogNapadaciTima dialogNapadaciTima, Tim tim)
        {
            napadaci = VratiNapadaceTima(tim);
            if(napadaci.Count > 0)
            {
                dialogNapadaciTima.DataGridNapadaci.DataSource = napadaci;
            }
            else
            {
                MessageBox.Show("Nema napadaca za tim " + tim.NazivTima);
                dialogNapadaciTima.Close();
            }
        }

        private BindingList<Igrac> VratiNapadaceTima(Tim tim)
        {
            List<Object> lista = Komunikacija.Komunikacija.Instance.VratiListu(Zajednicki.Operacije.VratiListuIgraca);
            napadaci = new BindingList<Igrac>();
            foreach ( Igrac i in lista )
            {
                if(i.TimID.TimID == tim.TimID && i.Pozicija == "Napad")
                {
                    napadaci.Add(i);
                }
            }
        ...
        internal void OtvoriDialogKreirajDrzavu()
        {
            dialogDodajDrzavu = new DialogDodajDrzavu(this);
            dialogDodajDrzavu.ShowDialog();
        }

        internal void InicijalizujDialogDodajDrzavu(DialogDodajDrzavu dialogDodajDrzavu)
        {
            this.dialogDodajDrzavu = dialogDodajDrzavu;
        }

        internal void DodajDrzavu()
        {
            if(string.IsNullOrEmpty(dialogDodajDrzavu.TxtNazivDrzave.Text))
            {
                MessageBox.Show("Polje za naziv drzave je obavezno!");
                return;
            }
            else
            {
                Drzava drzava = new Drzava();
                drzava.NazivDrzave = dialogDodajDrzavu.TxtNazivDrzave.Text;
                Drzava pom = (Drzava)Komunikacija.Komunikacija.Instance.Kreiraj(Zajednicki.Operacije.KreirajDrzavu, drzava);
                if (pom != null)
                {
                    MessageBox.Show($"Drzava {drzava.NazivDrzave} je uspešno kreirana!");
                }
                else
                {
                    MessageBox.Show("Sistem ne može da zapamti unetu drzavu!");
                }
                dialogDodajDrzavu.Dispose();
            }
        }
